Public Class Cuentabancaria
    ...
    Private Sub Insercionymodificacion()
        ...
        SERVIDORMYSQL.INSERTCOMMANDSQL.CommandText = "Insert Into `CuentaBancaria` " &
    ...
    Private Sub Boton_aceptar_Click(sender As Object, e As EventArgs) Handles Boton_aceptar.Click
        ...
        Dim Messagedeerror As String = ""
        Select Case Chequeinicial_textbox.TextLength
            Case = 8
                datoscorrectos += 1
            Case = 0
                '   Messagedeerror = Messagedeerror & "Debe ingresar el número del primer cheque, por favor verifique"
            ...
            Case = 1
                ...
                datoscorrectos += 1
            Case Else
                Messagedeerror += "el número de cheque debe contener exactamente 10 números, por favor verifique"
        End Select
        '1
        Select Case Cuenta_textbox.TextLength
            Case = 15
                datoscorrectos += 1
            Case = 0
                Messagedeerror += vbCrLf & "Es imprescindible que ingrese el número de Cuenta, por favor ingreselo"
            Case Else
                Messagedeerror += vbCrLf & "el número de cuenta debe contener   exactamente 15 caracteres, por favor verifique"
        End Select
        '2
        ...
        Select Case datoscorrectos
            ...
            Case Else
                MessageBox.Show(Messagedeerror & datoscorrectos)
        End Select
    End Sub
    ...
    Private Sub Boton_borrar_Click(sender As Object, e As EventArgs) Handles Boton_borrar.Click
        'numero de cuenta bancaria Cuenta_textbox
        SERVIDORMYSQL.INSERTCOMMANDSQL.Parameters.AddWithValue("@Cuenta", Cuenta_textbox.Text)
        SERVIDORMYSQL.INSERTCOMMANDSQL.CommandText = "DELETE from `CuentaBancaria` " &
        " WHERE cuenta=@cuenta"
        Inicio.INSERTSQLPARAMETROS(Autorizaciones.Organismotabla, System.Reflection.MethodBase.GetCurrentMethod.Name)
        Refreshnow()
    End Sub
    ...
    'Private Sub Fecha_apertura_textbox_TextChanged(sender As Object, e As EventArgs) Handles Fecha_apertura_textbox.TextChanged
    '    Inicio.Verificar(sender, sender.text, "FECHA")
    '    Select Case IsDate(sender.text)
    '        Case True
    '            Select Case Convert.ToDateTime(sender.text).Year > 1999
    '                Case True
    '                    Select Case Calendariomensual.SelectionStart = Convert.ToDateTime(sender.text)
    '                        Case True
    '                        Case False
    '                            Calendariomensual.SelectionStart = Convert.ToDateTime(sender.text)
    '                            Calendariomensual.SelectionEnd = Convert.ToDateTime(sender.text)
    '                    End Select
    '                Case False
    '            End Select
    '        Case False
